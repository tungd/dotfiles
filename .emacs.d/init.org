...
#+begin_src emacs-lisp
  (use-package deadgrep
    :ensure t
    :defer t
    :bind (:map projectile-command-map ("C-s" . deadgrep)))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package poetry
    :ensure t
    :defer t
    ;; :hook (python-mode . poetry-track-virtualenv)
    :config
    (eval-and-compile
      (defun td/poetry-get-virtualenv ()
        (poetry-ensure-in-project)
        (let ((buf (poetry-call 'show '("-v") nil nil t)))
          (with-current-buffer buf
            (goto-char (point-min))
            (re-search-forward "virtualenv: " nil t)
            (buffer-substring-no-properties (point) (line-end-position)))))
      (advice-add 'poetry-get-virtualenv :override #'td/poetry-get-virtualenv)))
#+END_SRC
