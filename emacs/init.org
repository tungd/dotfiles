...
slow. J/k, this has been fixed and Emacs has no problem handling these files
now.
...
Dumb-jump, as fallback for languages that their LSP server doesn't work well,
and/or languages where I need to browse the code but don't necessarily want to
...
#+begin_src emacs-lisp :tangle no
    (defun td/adapt-font-size (&optional frame)
      (let* ((attrs (frame-monitor-attributes frame))
             (geometry (alist-get 'geometry attrs)) ;; x y w h
             (size (alist-get 'mm-size attrs)) ;; w h
