...
#+begin_src emacs-lisp :tangle no
  (use-package lspce
    :load-path "/Users/tung/.config/emacs/vendor/lspce"
    :hook
    ((js-ts-mode . lspce-mode)
    (typescript-ts-mode . lspce-mode)
    (tsx-ts-mode . lspce-mode)
    (go-ts-mode . lspce-mode)
    (ocaml-ts-mode . lspce-mode)
    (kotlin-ts-mode . lspce-mode)
    (scala-ts-mode . lspce-mode)
    (tuareg-mode . lspce-mode))
    :custom
    (lspce-send-changes-idle-time 0.05)
    (lspce-show-log-level-in-modeline t)
    (lspce-inherit-exec-path t)
    ...
    (add-to-list 'lspce-server-programs '("scala" "metals" ""))
    (add-to-list 'lspce-server-programs '("typescriptreact" "typescript-language-server" "--stdio"))
    (add-to-list 'lspce-server-programs '("tuareg" "ocamllsp" ""))
    (add-to-list 'lspce-server-programs '("ocaml" "ocamllsp" "")))
#+end_src
...
#+begin_src emacs-lisp :tangle no
  (use-package aider
    ...
    (aider-args `("--model" "anthropic/claude-3-5-sonnet-20241022"
                  "--cache-prompts" "--no-stream" "--watch-files"
                  "--no-auto-commits"
                  "--api-key"
                  ,(format "anthropic=%s" (auth-source-pick-first-password :host "claude" :max 1)))))
#+end_src
...
#+begin_src emacs-lisp
  (use-package aidermacs
    ...
    (aidermacs-extra-args `("--model" "anthropic/claude-3-5-sonnet-20241022"
                    "--cache-prompts" "--no-stream" "--watch-files"
                    "--no-auto-commits"
                    "--api-key"
                    ,(format "anthropic=%s" (auth-source-pick-first-password :host "claude" :max 1)))))
