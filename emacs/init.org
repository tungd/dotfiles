...
#+begin_src emacs-lisp
  (use-package project
    :commands (project-find-file project-root project-current)
...
#+begin_src emacs-lisp
  ...
  (use-package consult
    ...
    (consult-project-root-function #'td/project-current-root))
...
I use =shackle= to manage popup windows. It was pulled from MELPA so I have it
installed in =vendor=. Currently I only one rule that make =compilation-mode=
windows pop under the current window and don't select it. In the future, I might
want to change this to be dynamic based on the result of the compilation, using
the =:custom= action.
...
#+begin_src emacs-lisp :tangle no
  (use-package shackle
    ...
    (shackle-rules `((compilation-mode :noselect t :size 0.2)
                     (ag-mode :select t)
                     ("*eldoc*" :noselect t))))
#+end_src
...
Separate the kill-ring from system clipboard.
...
#+begin_src emacs-lisp :tangle no
  (use-package corfu
    ...
    :bind* (:map corfu-map ("C-M-h" . corfu-show-documentation)))
...
#+BEGIN_SRC emacs-lisp
  (defun td/lsp-pyright ()
    (require 'lsp-pyright)
    (lsp))

  (use-package lsp-pyright
    :ensure t
    :hook (python-mode . #'td/lsp-pyright))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package doom-themes
    ...
    :hook ((org-mode . doom-themes-org-config)
           (org-mode . doom-themes-visual-bell-config)))
#+END_SRC

#+BEGIN_SRC emacs-lisp
  (global-eldoc-mode -1)
  (bind-key "C-c l" (lambda () (interactive) (eldoc-print-current-symbol-info)))
#+END_SRC
