...
#+begin_src emacs-lisp
  ...
  (use-package gptel
    ...
    :hook ((gptel-mode . visual-line-mode)
           (gptel-mode . td/gptel-setup-directory-indicator))
    :custom
    (gptel-model 'glm-4.7)
    (gptel-default-mode 'org-mode)
    :config
    (defun td/gptel-directory-name ()
      "Get a shortened directory name for gptel header."
      (let ((dir default-directory))
        (cond
         ;; Check if in a project
         ((bound-and-true-p project-current)
          (file-name-nondirectory (directory-file-name 
                                   (cdr (project-current)))))
         ;; Show last 2 directory components
         (t
          (let ((dirs (split-string (directory-file-name dir) "/")))
            (if (> (length dirs) 1)
                (string-join (last dirs 2) "/")
              (car dirs)))))))

    (defun td/gptel-setup-directory-indicator ()
      "Add current working directory to gptel header line."
      (when gptel-use-header-line
        (setq-local header-line-format
                    (append header-line-format
                            (list
                             (propertize " • " 'face 'font-lock-comment-face)
                             (propertize (td/gptel-directory-name)
                                         'face 'font-lock-string-face
                                         'help-echo (expand-file-name default-directory)))))))
    :config
...
#+begin_src emacs-lisp
  (use-package notmuch
    ...
    (defun my/notmuch-show-view-html-xwidget ()
      "Find the HTML part, render in xwidget-webkit, and MAXIMIZE the window."
      (interactive)
      (let* ((msg (notmuch-show-get-message-properties))
        ...
        (if html-part
            (let ((coding-system-for-write 'utf-8)
              ...
              ;; FIX: Use 'notmuch-get-bodypart-text' and pass the full 'msg' object
